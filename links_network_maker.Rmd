...
autoNet<-st_read("data/ArcGisJoins/AutoNetwork/AutoNetwork.shp")%>%st_transform(4326)%>%mutate(link_id=Link_Num)
utaharea<-st_read("data/ArcGisJoins/utaharea/utaharea.shp")%>%
    sf::st_transform(4326)
#utah_net<-extract_roads(utaharea,"data/MM_NetworkDataset_03042021.gdb")
#write_linknodes(utah_net, "data/utah")
```

```{r}
#autoTest <- linksFile %>% filter(start_node == end_node)
#  #filter(link_id==30093)
#leaflet() %>%
#  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
#  addPolylines(data = st_zm(autoTest), group = "Capacities", color = "blue")%>%
#  #addPoints(data = mynodes, group = "No Capacities", color = "red")%>%
...
```{r}
  ...
  nodes <- sf::st_read("data/MM_NetworkDataset_03042021.gdb", layer = "NetworkDataset_ND_Junctions") %>%
    sf::st_transform(4326) %>%
    # create a node id
    dplyr::mutate(id = dplyr::row_number()) %>%
    sf::st_filter(utaharea)
...
st_geometry(mylinks) <- NULL
  ...
  mynodes %>%
    ...
    readr::write_csv(file.path("data/utah", "nodes.csv"))
```
...
to_from <- mylinks %>% select(link_id,start_node,end_node)
amb <- st_read("data/ArcGisJoins/auto_master_basic_agrc/auto_master_basic.shp") %>% 
  mutate(link_id=Link_Num) %>%
  select(link_id,SEGID,Name,Oneway,Speed,DriveTime,Length_Mil,RoadClass,AADT) %>%
  st_transform(4326)
auto_master_basic <-left_join(amb,to_from,by="link_id") %>%
  select(link_id,SEGID,Name,Oneway,Speed,DriveTime,Length_Mil,RoadClass,AADT, start_node, end_node)
```
...
freeways <- st_read("data/ArcGisJoins/auto_master_freeway_agrc/auto_master_freeway.shp") %>% 
  mutate(link_id=Link_Num) %>%
  select(link_id,SEGID,Name,Oneway,Speed,DriveTime,Length_Mil,RoadClass,AADT) %>%
...
auto_tdm_ramps <- st_read("data/ArcGisJoins/auto_tdm_ramps_agrc/auto_tdm_ramps.shp") %>% 
  mutate(link_id=Link_Num) %>%
...
auto_master_joint <- left_join(auto_master_fix,seg_summaries,by = "SEGID")
...
leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addPolylines(data = st_zm(freewaycaps), group = "Capacities", color = "blue")%>%
...
leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addPolylines(data = st_zm(rampscaps), group = "Capacities", color = "blue")%>%
  #addPolylines(data = nocaps, group = "No Capacities", color = "red")%>%
...
leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addPolylines(data = st_zm(loc_high), group = "Local Roads w/ FW Values", color = "red") %>%
  addPolylines(data = st_zm(fw_low), group = "FW w/ Local Roads Values", color = "blue") %>%
...
leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addPolylines(data = st_zm(clct), group = "Collectors", color = "red") %>%
  addPolylines(data = st_zm(exp), group = "Expressways", color = "orange") %>%
  addPolylines(data = st_zm(fw), group = "Freeways", color = "yellow") %>%
  addPolylines(data = st_zm(loc), group = "Local Roads", color = "green") %>%
  addPolylines(data = st_zm(ma), group = "Minor Arterials", color = "blue") %>%
  addPolylines(data = st_zm(pa), group = "Principal Arterials", color = "purple") %>%
  addPolylines(data = st_zm(badloc), group = "Local Roads w/ Freeway Vals", color = "pink") %>%
...
cap_ln_auto_master <- ftclean_auto_master %>%
  mutate(
    CAP1HL = case_when(
      ...
      is.na(CAP1HL) & FTCLASS == "Expressway" ~ 978,
      is.na(CAP1HL) & FTCLASS == "Freeway" ~ 1772,
      is.na(CAP1HL) & FTCLASS == "Minor Arterial" ~ 865,
      is.na(CAP1HL) & FTCLASS == "Principal Arterial" ~ 915,
      is.na(CAP1HL) & FTCLASS == "Local" ~ 462,
...
leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addPolylines(data = st_zm(na_onewway), group = "NA", color = "red") %>%
  addPolylines(data = st_zm(tf_onewway), group = "TF", color = "blue") %>%
...
one_auto_master <- cap_ln_auto_master %>%
  mutate(
    to_node = end_node, from_node = start_node,
    start_node = case_when(
      is.na(Oneway) ~ to_node,
      is.na(Oneway)==FALSE ~ from_node),
    end_node = case_when(
      is.na(Oneway) ~ from_node,
      is.na(Oneway)==FALSE ~ to_node),
    Oneway = case_when(
      Oneway == "TF" ~ 1,
      Oneway == "B" ~ 0,
      is.na(Oneway) ~ 1)
...
write_csv(linksFileExcel,"data/utah/links.csv")
```
