...
## Ramp Problem
There is a problem where the agents aren't using the ramps. Lets solve that ish

read in nodes
```{r}
nodeshp <- st_read("data/ArcGisJoins/AutoNetwork/nodes.shp") %>% st_transform(4326)
leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  addMarkers(data = nodeshp, group = "Capacities")%>%
  #addPolylines(data = nocaps, group = "No Capacities", color = "red")%>%
  addLayersControl(overlayGroups = c("Roadways with Capacities"))
```

determine nodes that are part of problem
```{r}
ramp_s <- one_auto_master %>% group_by(start_node) %>% filter(n()>1)  %>% filter(RC==7)
ramp_e <- one_auto_master %>% group_by(end_node) %>% filter(n()>1)  %>% filter(RC==7)

badstartlinks <- one_auto_master %>% group_by(start_node) %>% filter(n()>1) %>% filter(FTCLASS=="Freeway") %>% filter(start_node %not in% ramp_s$start_node)
badendlinks <- one_auto_master %>% group_by(end_node) %>% filter(n()>1) %>% filter(FTCLASS=="Freeway") %>% filter(end_node %not in% ramp_e$end_node)

badstartnodes <- nodeshp %>% filter(node_id %in% badstartlinks$start_node)
badendnodes<- nodeshp %>% filter(node_id %in% badendlinks$end_node)

leaflet() %>%
  addProviderTiles(providers$Esri.WorldGrayCanvas) %>%
  #addMarkers(data = badnodess, group = "Capacities")%>%
  addMarkers(data=badstartnodes,group="caps")%>%
  addMarkers(data=badendnodes,group="caps")%>%
  #addPolylines(data = badstartlinks, group = "Capacities", color = "red")%>%
  #addPolylines(data = badendlinks, group = "Capacities2", color = "blue")%>%
  addLayersControl(overlayGroups = c("Freeways"))
```

create a set of new set of nodes
```{r}
#newnodes <- badnodess %>% mutate(node_id = paste(node_id,sep="","_2"))
newnodes <- badendnodes %>% mutate(node_id = node_id+1000000000)
newnodes2 <- badstartnodes %>% filter(node_id %not in% (badendnodes$node_id))%>% mutate(node_id = node_id+1000000000)
```

create the full nodes excel output that has all nodes on it
```{r}
st_geometry(nodeshp) <- NULL
st_geometry(newnodes) <- NULL
st_geometry(newnodes2) <- NULL
fullnodes<- rbind(nodeshp,newnodes) 
fullnodes2<- rbind(fullnodes,newnodes2) %>% mutate(id=node_id,x=xcoord,y=ycoord)%>%select(id,x,y)
write_csv(fullnodes2,"data/utah/nodes_ramps.csv")
```

replace links in og file that match the ones found above to replace their start nodes with the ones just created
```{r}
one_auto_master2 <- one_auto_master %>%
  mutate(
    start_node = case_when(
      #link_id %in% badstartlinks$link_id ~ paste(start_node,sep="","_2"),
      link_id %in% badstartlinks$link_id ~ start_node+1000000000,
      link_id %not in% badstartlinks$link_id ~ start_node
    ),
    end_node = case_when(
      #link_id %in% badendlinks$link_id ~ paste(end_node,sep="","_2"),
      link_id %in% badendlinks$link_id ~ end_node+1000000000,
      link_id %not in% badendlinks$link_id ~ end_node
    )
  )
```
...
write_csv(linksFileExcel,"data/utah/links_ramps.csv")
```
